...
# Функция callback для обработки данных из очереди
def callback(ch, method, properties, body):
    ...
    if not any(log_df.loc[message_id, ['y_true', 'y_pred']].isna()):
        # Вычисляем accuracy для данной выборки
        y_true = log_df.loc[message_id, 'y_true'] 
        y_pred = log_df.loc[message_id, 'y_pred']
        accuracy = accuracy_score([y_true], [y_pred])

        # Заносим accuracy в таблицу
        log_df.loc[message_id, 'accuracy'] = accuracy

        # Логируем истинный ответ, предсказание и accuracy в таблицу       
        with open('logs/metric_log.csv', 'a', newline='') as csvlog:
            writer = csv.writer(csvlog, delimiter=',')
            writer.writerow([message_id, y_true, y_pred, accuracy])
